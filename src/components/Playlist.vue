<template>
  <div>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div v-for="playlist in playlists" :key="playlist.id" class="bg-white dark:bg-gray-700 rounded-lg shadow-md p-4">
        <h3 class="text-xl font-semibold mb-2 text-gray-900 dark:text-white">{{ playlist.name }}</h3>
        <p class="text-gray-600 dark:text-gray-300 mb-2">{{ playlist.videos.length }} video</p>
        <button @click="viewPlaylist(playlist)" class="btn btn-secondary mr-2">Visualizza</button>
        <template v-if="isAdminUser">
          <button @click="editPlaylist(playlist)" class="btn btn-secondary mr-2">Modifica</button>
          <button @click="deletePlaylist(playlist.id)" class="btn btn-danger">Elimina</button>
        </template>
      </div>
    </div>
    ...
    <div v-if="showModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-3xl w-full">
        ...
        <input v-model="playlistName" placeholder="Nome playlist" class="w-full mb-4 p-2 border rounded">
        <div class="mb-4">
          <h3 class="text-lg font-semibold mb-2 text-gray-900 dark:text-white">Seleziona video</h3>
          <div class="max-h-60 overflow-y-auto">
            <div v-for="video in availableVideos" :key="video.id" class="flex items-center mb-2">
              <input type="checkbox" :id="video.id" v-model="selectedVideos" :value="video.id" class="mr-2">
              <label :for="video.id" class="text-gray-700 dark:text-gray-300">{{ video.title }}</label>
            </div>
          </div>
        </div>
    ...
    <div v-if="showPlaylistModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-white dark:bg-gray-800 p-6 rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div class="flex justify-end mt-4">
          <button @click="closePlaylistModal" class="btn btn-secondary">Chiudi</button>
        </div>
      </div>
    </div>
  </div>
    ...
    const savePlaylist = async () => {
      ...
      try {
        if (editingPlaylist.value) {
          await axios.put(`http://localhost:3000/playlists/${editingPlaylist.value.id}`, playlistData);
        } else {
